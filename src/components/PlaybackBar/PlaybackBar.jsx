...
export default function PlaybackBar({ isPlaying, animatingId, onPlay, onPlayAll, onStop }) {
    ...
    <div className="playback-bar">
      ...
      <div className="playback-controls">
        ...
        <button
          ...
          title={isPlaying ? 'Stop' : 'Play queued routes'}
        >
          {isPlaying ? '⏹' : '▶'}
        </button>
        ...
        {!isPlaying && (
          <button
            className="playback-play-btn play-all-btn"
            onClick={onPlayAll}
        ...
        <div className="playback-queue">
          {queue.length === 0
            ? <span className="playback-queue-empty">☆ Queue routes in the Routes tab, then hit ▶ to play in sequence</span>
            : queue.map((route) => (
              <div
                key={route.id}
        ...
        {isPlaying && currentRoute && (
          <div className="playback-current-progress">
            ...
            <span className="playback-step-info">{Math.round(currentProgress * 100)}%</span>
          </div>
        )}
